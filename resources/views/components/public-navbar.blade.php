<div class="bg-secondary w-full text-gray-400">
    <div x-data="{ open: false }"
        ...
        <div class="flex flex-row items-center justify-between p-4">
            ...
            <button class="md:hidden focus:outline-none focus:shadow-outline rounded-lg text-action"
                @click="open = !open">
        ...
        <nav :class="{'flex': open, 'hidden': !open}"
            ...
            {{-- <div @click.away="open = false" class="relative" x-data="{ open: false }">
				...
				<div x-show="open" x-transition:enter="transition ease-out duration-100"
					...
					<div class="dark-mode:bg-gray-700 px-2 py-2 bg-white rounded-md shadow">
            ...
            @if (Auth::check())
                <div @click.away="open = false" class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        class="md:w-auto md:inline md:mt-0 md:ml-4 flex flex-row items-center w-full px-2 py-2 mt-2 font-semibold text-left bg-transparent rounded-lg">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->display_name) }}"
                            class="inline rounded-full" width="30" height="30" alt="">
                        <svg fill="currentColor" viewBox="0 0 20 20" :class="{'rotate-180': open, 'rotate-0': !open}"
                            class="md:-mt-1 inline w-4 h-4 mt-1 ml-1 transition-transform duration-200 transform">
                            <path fill-rule="evenodd"
                                d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-100"
                        ...
                        <div class="px-2 py-2 bg-white rounded-md shadow">
                            ...
                            <a class="md:mt-0 hover:bg-gray-200 block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg"
                                href="{{ route('auth.account.show', Auth::user()) }}">Your Account</a>

                            <?php
                            if (auth()->user()->account_status == 1) {
                                if (auth()->user()->reapply == 'Y') {
                                    if (auth()->user()->reapply_date <= date('Y-m-d')) {
                                        echo '<a href=' . route('home') . " class=\"md:mt-0 hover:bg-gray-200 block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg\">Apply For Membership</a>";
                                    }
            ...
            @else
                @if (session('steam_hex'))
                    <a class="md:mt-0 hover:text-white px-4 py-2 mt-2 font-semibold text-gray-400 rounded-lg {{ Request::is('account/create') ? 'text-white' : '' }}"
                        href="{{ route('auth.account.create') }}">About
                        You</a>
                @else
                    <a class="md:mt-0 hover:text-white px-4 py-2 mt-2 font-semibold text-gray-400 rounded-lg"
                        href="{{ route('auth.steam') }}">Login</a>
                @endif
            @endif

            <a class="md:mt-0 hover:text-white px-3 py-2 mt-3 font-semibold rounded-lg {{ Request::is('/') ? 'text-white' : '' }}"
                href="{{ route('home') }}"><label for="toggle" class="dark:text-yellow-400 text-action">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                        x-show="darkMode" stroke="currentColor" stroke-width="2">
